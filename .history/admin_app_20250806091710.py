...
def admin_create_user():
    if request.method == 'POST':
        ...
        # Create new user
        user = User(email=email, name=name, company=company, role=role)
        db.session.add(user)
        db.session.commit()
...
def admin_assessments():
    page = request.args.get('page', 1, type=int)
    assessments = Assessment.query.order_by(Assessment.created_at.desc()).paginate(
        page=page, per_page=20, error_out=False)
...
def admin_create_assessment():
    if request.method == 'POST':
        ...
        is_self_assessment = 'is_self_assessment' in request.form
        creator_id = request.form.get('creator_id', type=int)
        
        if not title:
            flash('Assessment title is required!', 'error')
            users = User.query.filter_by(is_active=True).all()
            return render_template('admin_create_assessment.html', users=users)
